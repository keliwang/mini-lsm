...
impl SimpleLeveledCompactionController {
    ...
    pub fn generate_compaction_task(
        ...
        let mut level_sizes = Vec::with_capacity(self.options.max_levels);
    ...
    pub fn apply_compaction_result(
        ...
        if let Some(upper_level) = task.upper_level {
            assert_eq!(
                ...
                "upper level ssts mismatch"
            );
            ssts_to_remove.extend(&snapshot.levels[upper_level - 1].1);
            snapshot.levels[upper_level - 1].1.clear();
        } else {
            ssts_to_remove.extend(&task.upper_level_sst_ids);
            let compacted_l0_ssts = task
                .upper_level_sst_ids
                .iter()
                .copied()
                .collect::<HashSet<_>>();
            let new_l0_ssts = snapshot
                ...
                .iter()
                .copied()
                .filter(|sst| !compacted_l0_ssts.contains(sst))
                .collect::<Vec<_>>();
            assert_eq!(
                snapshot.l0_sstables.len() - compacted_l0_ssts.len(),
                new_l0_ssts.len()
            );
