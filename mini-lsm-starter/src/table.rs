...
impl BlockMeta {
    ...
    pub fn decode_block_meta(buf: &[u8]) -> Result<(Vec<BlockMeta>, u64)> {
        ...
        let mut data = &buf[..checksum_offset];
        let checksum = (&buf[checksum_offset..]).get_u32();
        ...
        for _ in 0..len {
            ...
            let first_key =
                KeyBytes::from_bytes_with_ts(data.copy_to_bytes(first_key_len), data.get_u64());
            let last_key_len = data.get_u16() as usize;
            let last_key =
                KeyBytes::from_bytes_with_ts(data.copy_to_bytes(last_key_len), data.get_u64());
